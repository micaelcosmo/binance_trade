...
# --- TURN OFF AVISO DE CPU ---
logging.getLogger("unicorn_binance_websocket_api").setLevel(logging.CRITICAL)
# -----------------------------------
...
class BinanceAPIManager:
    ...
    def get_fee(self, origin_coin: Coin, target_coin: Coin, selling: bool):
        ...
        amount_trading = (
            self._sell_quantity(origin_symbol=origin_coin.symbol, target_symbol=target_coin.symbol)
            if selling
            else self._buy_quantity(origin_symbol=origin_coin.symbol, target_symbol=target_coin.symbol)
        )
    ...
    # --- NOVO: BUSCA O 24H DE TODAS AS MOEDAS COM CACHE DE 60S ---
    @cached(cache=TTLCache(maxsize=1, ttl=60))
    def get_all_tickers_24h(self) -> Dict[str, float]:
        try:
            tickers = self.binance_client.get_ticker()
            return {t['symbol']: float(t['priceChangePercent']) for t in tickers}
        except Exception as e:
            self.logger.error(f"Erro ao buscar tickers 24h: {e}")
            return {}
    # -------------------------------------------------------------
    ...
    def _wait_for_order(
        ...
        while True:
            ...
            espera_atual += 1
            if espera_atual > timeout_limit:
                print() 
                self.logger.warning(f"WebSocket ignorou a ordem {order_id}. Checando REST API...")
                
                try:
                    symbol_str = origin_symbol + target_symbol
                    fallback_order = self.binance_client.get_order(symbol=symbol_str, orderId=order_id)
                    
                    if fallback_order['status'] == 'FILLED':
                        self.logger.info(f"[OK] REST API confirmou: Ordem {order_id} executada.")
                        
                        if fallback_order['side'] == 'BUY':
                            from .models import Coin
                            self.db.set_current_coin(Coin(origin_symbol))
                        elif fallback_order['side'] == 'SELL':
                        ...
                        class MockOrder:
                            def __init__(self, data):
                                ...
                                if raw_price == 0.0:
                                    ...
                                    if exec_qty > 0:
                                        raw_price = cumm_quote / exec_qty
                                    else:
                                        fills = data.get('fills', [])
                                        if fills:
                                            raw_price = float(fills[0].get('price', 0.0))
                                            
                                if raw_price == 0.0:
                                    tk_price = manager_instance.get_ticker_price(self.symbol)
                                    raw_price = float(tk_price) if tk_price else 0.00000001
                                            
                                self.price = raw_price
                                self.time = data.get('time')
                                self.cumulative_quote_qty = float(data.get('cummulativeQuoteQty', 0.0))
                                
                        return MockOrder(fallback_order)
                        
                    elif fallback_order['status'] in ['NEW', 'PARTIALLY_FILLED']:
                        self.logger.info(f"Ordem {order_id} pendente. Cancelando para destravar...")
                        self.binance_client.cancel_order(symbol=symbol_str, orderId=order_id)
                except Exception as e:
                    self.logger.error(f"Erro no double-check: {e}")
                
                return None 

            print(f"\r[!] Aguardando {order_id}... [{espera_atual}/{timeout_limit}]", end="", flush=True)
            time.sleep(1) 
    ...
    def _buy_quantity(
        ...
        target_balance = target_balance * 0.985
    ...
    def _buy_alt(self, origin_coin: Coin, target_coin: Coin):
        ...
        while order is None:
            try:
                ...
                if e.code in [-1013, -2010]:
                    self.logger.warning("Binance rejeitou a quantidade. Abortando compra!")
                    return None
                time.sleep(1)
            except Exception as e:
        ...
        if float(getattr(order, 'price', 0.0)) == 0.0:
            tk_price = self.get_ticker_price(origin_symbol + target_symbol)
            order.price = float(tk_price) if tk_price else 0.00000001
    ...
    def _sell_alt(self, origin_coin: Coin, target_coin: Coin):
        ...
        while order is None:
            try:
                ...
                self.logger.info(e)
                if e.code in [-1013, -2010]:
                    self.logger.warning("Binance rejeitou a quantidade. Abortando venda!")
                    return None
                time.sleep(1)
            except Exception as e:
                self.logger.warning(f"Unexpected Error: {e}")
